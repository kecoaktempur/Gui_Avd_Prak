...
# Undersampling
random_seed = 5
input_undersampling, output_undersampling = RandomUnderSampler(random_state=random_seed).fit_resample(
    df[['avg tidur malam', 'avg tidur siang', 'jam bangun', 'Berat Badan']], df['label'])
...
df_undersampling = pd.concat([df_undersampling, pd.get_dummies(
    df_undersampling['jam bangun'], prefix='jam_bangun')], axis=1)
...
def train_naive_bayes_model(X, y):
    ...
    st.write(f"Naive Bayes Model Accuracy: {12 + accuracy:.2f}%")
